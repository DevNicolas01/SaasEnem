...
const styles = {
  ...
  timer:
    "flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm tabular-nums",
  ...
  textarea:
    "w-full h-96 bg-white/5 border border-white/10 rounded-3xl p-5 text-sm leading-relaxed text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-400/40 resize-none",

  finishedCard:
    "max-w-md w-full bg-white/5 border border-white/10 rounded-3xl p-8 text-center backdrop-blur",
  ...
  iaBox:
    "bg-gradient-to-br from-amber-400 to-orange-500 rounded-3xl p-5 text-slate-950 mb-5",
  ...
  secondary:
    "w-full bg-white/10 hover:bg-white/20 text-white font-semibold py-3 rounded-2xl transition",
};
        ...
        <div className="min-h-dvh flex items-center justify-center px-6">
          <div className={styles.finishedCard}>
            ...
            <p className={styles.finishedDescription}>
              Você escreveu {wordCount} palavras. Que tal ver como ficaria a
              correção dessa redação?
            </p>

            <div className={styles.iaBox}>
              <p className={styles.iaTitle}>🤖 Teste sua Nota com IA</p>

              <p className={styles.iaDescription}>
                Cole seu texto lá e receba a correção completa
              </p>
            </div>
  ...
  return (
    <main className={styles.page}>
      <div className={styles.container}>
        <button onClick={onExit} className={styles.back}>
          ← Sair do simulado
        </button>
        ...
        <div className="flex items-center justify-between mb-5">
          <div
            className={`${styles.timer} ${
              isLowTime ? styles.timerDanger : styles.timerNormal
            }`}
          >
            ⏱️ {formatted}
          </div>
        ...
        {!started ? (
          <section className={styles.startCard}>
            <p className={styles.text}>
              Você terá 1 hora, o mesmo tempo da prova real. Pronto para
              começar?
            </p>
